...
function handler(fn) {
  ...
  return async event => {
    let message; // Attempt to parse the message as JSON. The "Records" provided on the event
    ...
    } catch (err) {// If the request body can't be parsed as JSON we just pass undefined to
