...
const SQS_BATCH_DELETE_CHUNK_SIZE = 10; // Utility to make a Lambda function handler. Expects to be invoked in response
...
function handler(fn, config = {}) {
  ...
  if (!(config !== null && config !== void 0 && config.queueURL)) {
    throw new Error('Expected an SQS queue URL.');
  }

  const sqs = (config === null || config === void 0 ? void 0 : config.sqs) ?? new _awsSdk.default.SQS({
  ...
  return async event => {
    ...
    if (Array.isArray(records)) {
      const results = await Promise.allSettled(records.map(record => {
        ...
        try {
          message = JSON.parse(record === null || record === void 0 ? void 0 : record.body);
        } catch (err) {// If the request body can't be parsed as JSON we just pass undefined
      ...
      if (fulfilledPromises.length) {
        const batch = fulfilledPromises.map(({
          value
        }, i) => ({
          Id: i.toString(),
      ...
      if (rejectedPromises.length) {
        const reasons = rejectedPromises.map(promise => {
          var _promise$reason;

          return promise === null || promise === void 0 ? void 0 : (_promise$reason = promise.reason) === null || _promise$reason === void 0 ? void 0 : _promise$reason.message;
        }).join('\n');
